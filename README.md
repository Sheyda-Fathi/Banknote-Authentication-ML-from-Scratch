...
a 70/15/15 train/validation/test split, 5-fold cross-validation, paired t-tests
...
Training — for each class `c` and feature `i`:
- Calculate mean μ꜀,ᵢ
- Calculate variance σ²꜀,ᵢ
- Calculate prior P(y=c)

Prediction (using log-probabilities for numerical stability):
...
Feature discretization: continuous features discretized using the mean threshold.
- Value ≤ mean → 0 (low)
- Value > mean → 1 (high)
...
- Significance level: α = 0.05 (p < 0.05 → significant)

### 8. Learning Curves

Training sizes tested: 20%, 40%, 60%, 80%, 100% of the data. For each size,
5-fold cross-validation is run and mean accuracy ± std is recorded.
...
**Best algorithm:** KNN achieved the highest accuracy, with the lowest
...
1. **KNN is the best-performing algorithm** on this dataset. Its ability to
   capture complex decision boundaries without strong distributional
   assumptions gives it an advantage.
2. **Naive Bayes underperforms** compared to the other algorithms. The
   feature-independence assumption likely does not hold for banknote
   features, and the Gaussian assumption may not perfectly fit the data.
3. **Decision Tree (depth=5)** achieves reasonable accuracy but shows higher
   variance than KNN. Pre-pruning helps prevent overfitting but may limit
   the model's ability to capture complex patterns.
4. **Statistical significance** is confirmed by paired t-tests — all
   performance differences between algorithms are statistically significant
   (p < 0.05).
5. **Learning curve analysis:**
   - Naive Bayes: stable, low bias
   - KNN: very stable, low variance, strong even with little data
...
**Naive Bayes**
- Assumes feature independence (may not hold here)
- Gaussian distribution assumption may be violated
- Lower accuracy compared to KNN and Decision Tree
...
- Discretization with a mean threshold loses information
- More variance than KNN on this dataset
- Can overfit without proper pruning

---

## Suggestions for Improving the Weakest Algorithm

Since **Naive Bayes** is the weakest algorithm on this dataset:

1. Try Kernel Density Estimation instead of a Gaussian assumption
2. Apply feature selection to remove redundant features
3. Use a data transformation (Box-Cox) to make features more Gaussian
...
- No data leakage: normalization parameters come from training data only
- Complete evaluation: cross-validation + t-test + learning curves
...
| `algorithm_comparison.png`           | Bar chart with error bars         |
| `learning_curves_report.png`         | Clean version for the report      |
| `learning_curve_naive_bayes.png`     | Naive Bayes learning curve        |
| `learning_curve_knn_(k=X).png`       | KNN learning curve (best k)       |
| `learning_curve_decision_tree.png`   | Decision Tree learning curve      |

---

## Conclusion

This project implements and evaluates three machine learning algorithms from
scratch on the banknote authentication dataset. KNN proved to be the most
effective classifier, achieving near-perfect accuracy. The systematic
evaluation using cross-validation and statistical testing provides
...
This project was submitted as coursework. All code is original and written
from scratch, without using pre-built machine learning libraries.